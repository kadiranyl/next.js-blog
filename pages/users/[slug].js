    ...
    if (usersArray.map(blog => blog.link == slug) && user && blogs) {
        return (
            ...
            <div className='w-[80%] md:w-[40%] mx-auto flex flex-col items-center gap-16 justify-center my-6'>
                <div className='flex items-center flex-col s sm:flex-row justify-start gap-8 w-full'>
                  <Image width="96px" height="96px" src={user.imageUrl} alt="" className='object-cover flex-none rounded-full' />
                  <div className='flex flex-col justify-center sm:max-w-[70%]'>
                  ...
                  <div className="flex flex-col items-center h-full w-full gap-6">
                    ...
                    <div className="flex justify-between gap-9 w-full items-center" key={blog.id}>
                        ...
                        <Link href={categoriesArray.find(category => category.id === blog.category) ? "/categories/" + categoriesArray.find(category => category.id === blog.category).link : "#"}>
                          <a className="text-black border-2 font-bold uppercase border-black w-fit px-3 py-[5px] text-xs mb-2">{categoriesArray.find(category => category.id === blog.category) ? categoriesArray.find(category => category.id === blog.category).name : "Deleted Category"}</a>
                        </Link>
                        <Link href={"/blogs/" + blog.link}>
